...
begin
  ...
  begin
    Close;
    CommandText := '';
    CommandText := 'select * from LANCAMENTO where DTLANCAMENTO = ' + #39 + edt_DataAbertura.Text + #39;
//    CommandText := 'select ID, CDFICHA, NOMEDIZ, Sum(VLRDOACAO) as TOTAL from LANCAMENTO where DTLANCAMENTO = ' + #39 + edt_DataAbertura.Text + #39 +
//                   'group by ID, CDFICHA, NOMEDIZ, VLRDOACAO';
    Open;
    Soma := 0;
    First;
...
//   lbl_Totaliza.Caption := FormatFloat('###,##0.00',Soma);
     lbl_TotalDizimistas.Caption := 'Nº Lancamentos: '+IntToStr(TClientDataSet(ds_TabelaConsulta.DataSet).RecordCount);
   end;
