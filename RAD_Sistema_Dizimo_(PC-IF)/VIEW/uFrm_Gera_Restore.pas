...
begin
  ...
  begin
    Application.MessageBox('Atenção, o Arquivo de Backup não foi informado.','Atenção.:', MB_APPLMODAL + MB_OK + MB_ICONWARNING);
    EXIT;
  end;

  if edt_FDB.Text = '' then
  begin
    Application.MessageBox('Atenção, local do Banco não foi informado para efetuar a Restauração.','Atenção.:', MB_APPLMODAL + MB_OK + MB_ICONWARNING);
    EXIT;
  end;
  ...
  try
    ...
    begin
      ...
      mem_Process.Lines.Add('');
      mem_Process.Lines.Add('');
      mem_Process.Lines.Add('Restauração do Banco efetuado com sucesso!!!!');
