...
begin
  with DM_Relatorios do
    ...
    try
      ...
      rel_Lancam.Variables['VlrTotal_Mes']  := QuotedStr(lbl_ValorTotalPeriodo2.Caption);
  ...
  begin
      ...
      begin
       begin
        ...
        begin
          Application.MessageBox('Data Final NÃO PODE SER MENOR que a Data Inicial!','Atenção',0+48);
          edt_DT_Inicial.SetFocus;
          Abort;
        end
        else
        ...
        if RecordCount = 0 then
          begin
            Application.MessageBox('Não Existe Dados no Período Solicitado!','Atenção',0+48);
            edt_DT_Inicial.SetFocus;
            Abort;
          end else
          First;
        ...
        lbl_ValorTotalPeriodo2.Caption   := 'R$ ' + FormatFloat('###,##0.00',Soma1);
       end;


        if DateToStr(dt_Fim) = '' then
        begin
          Application.MessageBox('Entre com uma data para a realização do cálculo!','Atenção',0+48);
          edt_DT_Inicial.SetFocus;
          Abort;
        end
        else
        ...
        lbl_ValorMes2.Caption     := 'R$ ' + FormatFloat('###,##0.00',Soma);
      end;
  end;
end;
